...
class AWS3Storage:
    def __init__(self, logger):
        ...
        self.bucket = 'cloudcomputinghomework01'
        self.bucket_location = self.s3.get_bucket_location(Bucket=self.bucket)['LocationConstraint']
    ...
    def upload_to_aws(self, local_file, key):
        ...
        try:
            ...
            file_object = [fo for fo in boto3.resource(
                's3',
                aws_access_key_id=self.ACCESS_KEY,
                aws_secret_access_key=self.SECRET_KEY).Bucket(self.bucket)
                .objects.all() if fo.key == key][0]

            reponse_ignored = file_object.Acl().put(ACL='public-read')  # TODO: parse this!

            url = "https://s3-{}.amazonaws.com/{}/{}".format(self.bucket_location, self.bucket, key)
